  ...
  return (
    <Route selected={props.selected}>
      <Box display="flex" justifyContent="space-between" alignItems="center" pb={30}>
        <Text color="text1" fontSize={[24, 20]} fontWeight={600}>
          ...
          return (
            <Box key={index}>
              <Box display={'flex'} flexDirection={'row'} alignItems="center" pb={'0.5rem'}>
                <Anchor size={20} color={theme.text1} />
                <Text pl={18} color="text1" fontSize={[16, 14]} fontWeight={600}>
                  {step.contractType}
                </Text>
              </Box>
              <StepDetail lastItem={index === props.steps.length - 1}>
                <Text color="text1" fontSize={[16, 14]} fontWeight={400} pb={'0.2rem'}>
                  {t('bridge.bridgeRoute.singleTransaction')}
                </Text>
                {step.subSteps.map((subStep, index) => {
                  return (
                    <Text pl={24} key={index} color="text1" fontSize={[16, 14]} fontWeight={400} pb={'0.2rem'}>
                      {subStep}
                    </Text>
                  );
                })}
      ...
      <Box pb={10}>
        <Text color={'text1'} fontSize={[16, 14]} fontWeight={600} pb={'0.2rem'}>
          {t('bridge.bridgeRoute.estimatedToken', { token: props.estimatedToken })}
        </Text>
        <Text color={'text1'} fontSize={[16, 14]} fontWeight={600}>
