...
import { NATIVE, RANGO_API_KEY, ZERO_ADDRESS } from 'src/constants';
...
export const getRangoRoutes: GetRoutes = async ({
  ...
  const fromCurrencyAddress =
    !fromCurrency.address || [ZERO_ADDRESS, NATIVE].includes(fromCurrency.address) ? null : fromCurrency.address;
  const toCurrencyAddress =
    !toCurrency.address || [ZERO_ADDRESS, NATIVE].includes(toCurrency.address) ? null : toCurrency.address;
  ...
  // no routes found
  if (rangoRouteRes?.resultType !== 'OK' || !rangoRouteRes?.route) return [];

  // routes founds but failed to create tx or pass validation checks
  if (rangoRouteRes?.resultType === 'OK' && !!rangoRouteRes?.error && !rangoRouteRes?.error?.includes('balance'))
    return [];
