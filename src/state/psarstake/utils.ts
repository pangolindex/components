...
export function formatPosition(
  ...
  const positions: (Position | undefined)[] = nftsURIs.map((uri, index) => {
    ...
    const _uri =
      uri ?? createURI(balance, valueVariables.sumOfEntryTimes, apr, pendingRewards, blockTimestamp, chainId);
...
export function createURI(
  ...
  const entryTime = balance.isZero() ? blockTimestamp : sumOfEntryTimes.div(balance);
