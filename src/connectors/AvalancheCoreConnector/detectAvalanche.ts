...
export function detectAvalancheProvider<T = AvalancheEthereumProvider>({
  ...
  function _validateInputs() {
    if (typeof mustBeAvalanche !== 'boolean') {
      throw new Error(`@avalanche/detect-provider: Expected option 'mustBeAvalanche' to be a boolean.`);
    }
    if (typeof silent !== 'boolean') {
      throw new Error(`@avalanche/detect-provider: Expected option 'silent' to be a boolean.`);
    }
    if (typeof timeout !== 'number') {
      throw new Error(`@avalanche/detect-provider: Expected option 'timeout' to be a number.`);
    }
  }
}
